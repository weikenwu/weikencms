...
<script type="text/javascript" src="../ckeditor/ckeditor.js"></script>
	...
	<body id="main">
		...
		<tr>
			...
			<td>{@value->count}</td>
			<td>{@value->date}</td>
			<td><a href="content.php?action=update&id={@value->id}">修改</a> | <a href="content.php?action=delete&id={@value->id}" onclick="return confirm('真的要删除文档吗?')?true:false">删除</a></td>
		</tr>
		{/foreach}
		</table>
		...
		<tr><td>文档标题：<input type="text" name="title" class="text" value="{$titlec}"> <span class="red">【必填】</span>（*标题2-50字符）</td></tr>
		<tr><td>tag标签：<input type="text" name="tag" class="text" value="{$tag}"> (*每个标签用“,”隔开，总长30位)</td></tr>
		<tr><td>栏 目：<select name="nav"><option value="">请选择栏目类别</option>{$nav}</select>  <span class="red">【必选】</span></td></tr>
		<tr><td>定义属性：{$attr}
					</td></tr>
		<tr><td>关 键 字：<input type="text" name="keyword" class="text" value="{$keyword}"> (*每个标签用“,”隔开，总长30位) </td></tr>
		<tr><td>缩 略 图：<input type="text" name="thumbnail" class="text" readonly="readonly" value="{$thumbnail}">
						<input type="button" value="上传缩略图" onclick="centerWindow('../templates/upfile.html','upfile',400,100)"/>
						<img name="pic" src="{$thumbnail}" style="display:block;"/> (*必须PNG,JPG,GIF，大小200K内)
		</td></tr>
		<tr><td>文章来源：<input type="text" name="source" class="text" value="{$source}">（*20字符内）</td></tr>
		<tr><td>作      者：<input type="text" name="author" value="{$author}" class="text">（*10字符内）</td></tr>
		<tr><td><span class="middle">内容摘要：</span><textarea name="info">{$info}</textarea><span class="middle">（*200字符内）</span></td></tr>
