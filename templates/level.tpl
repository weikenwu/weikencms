...
<script type="text/javascript" src="../js/admin_level.js"></script>
	...
	<body id="main">
		...
		<tr>
			...
			<td><a href="level.php?action=update&id={@value->id}">修改</a> | <a href="level.php?action=delete&id={@value->id}" onclick="return confirm('真的要删除等级吗?')?true:false">删除</a></td>
		</tr>
		{/foreach}
		</table>
		...
		<form action="" method="post" name="add">
			<table cellspacing="0" class="left">
				...
				<input type="checkbox" name="premission[]" value="{@value->id}" />{@value->name} 
				{/foreach}
				{/if}
				</td></tr>
